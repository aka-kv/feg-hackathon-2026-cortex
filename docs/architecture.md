...
3. **Warm (3+ distinct fixtures viewed) — two-stage retrieval + reranking:**
   - **Stage 1 (retrieval)**: `graph_candidates()` aggregates real co-occurrence edges from every fixture the session has seen, ranked by summed edge weight, restricted to the sport(s) already engaged with (see §4) — but pulls a pool of 15 candidates (`RETRIEVAL_POOL_SIZE`), not just the 3 that get shown. Backfilled with popularity candidates if the graph doesn't produce enough.
   - **Stage 2 (reranking)**: `rerank_candidates()` takes that pool and blends in a second, independent signal (global popularity, rank-normalized to `[0,1]` within the pool) via a tunable `shrinkage_k`, before cutting to `top_n`. Tested against the held-out evaluation before choosing a default, and the honest result was negative: any nonzero `shrinkage_k` causes an immediate, sharp accuracy drop (0.4214 → 0.3711 at just `k=0.01`) — the same root cause as the earlier, separately-rejected PMI/lift experiment (§Phase F of the design doc): co-occurrence counts are sparse enough that the raw ranking is usually decisive, and nudging it away from pure rank order costs real hits far more often than it fixes ties. **Shipped default is `RERANK_SHRINKAGE_K = 0.0`** — mathematically a no-op (verified: byte-identical held-out result to before the refactor) — the reranking stage is real, unit-tested, and available, but disabled because this specific signal measurably makes things worse, not because it wasn't tried. Full sweep table in `design-final-sports-graph.md` Phase F.
